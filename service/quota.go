...
func PostConsumeQuota(relayInfo *relaycommon.RelayInfo, quota int, preConsumedQuota int, sendEmail bool) (err error) {
	...
	if relayInfo != nil && relayInfo.BillingSource == BillingSourceSubscription {
		...
		if relayInfo.SubscriptionUsageRef.Valid() {
			target := relayInfo.SubscriptionPreConsumed + relayInfo.SubscriptionPostDelta + delta
			if target < 0 {
				target = 0
			}
			if err := model.SetSubscriptionUsageFinal(&relayInfo.SubscriptionUsageRef, target, model.SubscriptionUsageStateSettled); err != nil {
				return err
			}
		} else if delta != 0 {

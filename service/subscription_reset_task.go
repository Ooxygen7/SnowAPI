...
func runSubscriptionQuotaResetOnce() {
	...
	if time.Since(lastCleanup) >= subscriptionCleanupInterval {
		...
		taskRetentionSeconds := int64(constant.TaskTimeoutMinutes)*60 + int64(24*time.Hour/time.Second)
		if taskRetentionSeconds > retentionSeconds {
			retentionSeconds = taskRetentionSeconds
		}
		if _, err := model.CleanupSubscriptionPreConsumeRecords(retentionSeconds); err == nil {
			if _, windowErr := model.CleanupSubscriptionQuotaWindows(retentionSeconds); windowErr == nil {

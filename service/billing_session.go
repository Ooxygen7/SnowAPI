...
func (s *BillingSession) Settle(actualQuota int) error {
	...
	if !s.fundingSettled {
		...
		if sub, ok := s.funding.(*SubscriptionFunding); ok {
			s.relayInfo.SubscriptionUsageRef = sub.usageRef
		}
	}
	if delta == 0 {
		s.syncRelayInfo()
...
func (s *BillingSession) syncRelayInfo() {
	...
	if sub, ok := s.funding.(*SubscriptionFunding); ok {
		...
		info.SubscriptionUsageRef = sub.usageRef
		...
		info.SubscriptionFiveHourQuota = sub.FiveHourQuota
		info.SubscriptionFiveHourUsedAfterPreConsume = sub.FiveHourUsedAfter + int64(s.extraReserved)
	} else {
		info.SubscriptionId = 0
		info.SubscriptionUsageRef = relaycommon.SubscriptionUsageRef{}
		info.SubscriptionPreConsumed = 0
		info.SubscriptionFiveHourQuota = 0
		info.SubscriptionFiveHourUsedAfterPreConsume = 0
	}
}

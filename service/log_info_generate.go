...
func appendBillingInfo(relayInfo *relaycommon.RelayInfo, other map[string]interface{}) {
	...
	if relayInfo.BillingSource == "subscription" {
		...
		if relayInfo.SubscriptionUsageRef.PeriodWindowId > 0 {
			other["subscription_period_window_id"] = relayInfo.SubscriptionUsageRef.PeriodWindowId
		}
		if relayInfo.SubscriptionUsageRef.FiveHourWindowId > 0 {
			other["subscription_five_hour_window_id"] = relayInfo.SubscriptionUsageRef.FiveHourWindowId
		}
		...
		if relayInfo.SubscriptionFiveHourQuota > 0 {
			fiveHourUsed := relayInfo.SubscriptionFiveHourUsedAfterPreConsume + relayInfo.SubscriptionPostDelta
			if fiveHourUsed < 0 {
				fiveHourUsed = 0
			}
			fiveHourRemaining := relayInfo.SubscriptionFiveHourQuota - fiveHourUsed
			if fiveHourRemaining < 0 {
				fiveHourRemaining = 0
			}
			other["subscription_five_hour_total"] = relayInfo.SubscriptionFiveHourQuota
			other["subscription_five_hour_used"] = fiveHourUsed
			other["subscription_five_hour_remain"] = fiveHourRemaining
		}

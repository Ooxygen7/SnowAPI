...
func truncate(t *testing.T) {
	...
	t.Cleanup(func() {
		...
		model.DB.Exec("DELETE FROM subscription_quota_windows")
...
func TestAsyncTaskSettlementAndRefundUseOriginalSubscriptionWindows(t *testing.T) {
	truncate(t)
	ctx := context.Background()

	const userID, tokenID, channelID = 41, 41, 41
	seedUser(t, userID, 0)
	seedToken(t, tokenID, userID, "sk-window-task", 8000)
	seedChannel(t, channelID)
	now := model.GetDBTimestamp()
	plan := &model.SubscriptionPlan{
		Title:            "Async window plan",
		PriceAmount:      1,
	...
	require.NoError(t, model.DB.Create(sub).Error)
	preConsumed, err := model.PreConsumeUserSubscription("async-window-task", userID, "test-model", 0, 500)
	require.NoError(t, err)

	task := makeTask(userID, channelID, 500, tokenID, BillingSourceSubscription, sub.Id)
	usageRef := preConsumed.UsageRef
	task.PrivateData.SubscriptionUsageRef = &usageRef
	require.NoError(t, task.Insert())

	RecalculateTaskQuota(ctx, task, 200, "async window adjustment")
	var periodWindow model.SubscriptionQuotaWindow
	require.NoError(t, model.DB.First(&periodWindow, preConsumed.UsageRef.PeriodWindowId).Error)
	assert.Equal(t, int64(200), periodWindow.AmountUsed)
	var fiveHourWindow model.SubscriptionQuotaWindow
	require.NoError(t, model.DB.First(&fiveHourWindow, preConsumed.UsageRef.FiveHourWindowId).Error)
	assert.Equal(t, int64(200), fiveHourWindow.AmountUsed)

	RefundTaskQuota(ctx, task, "async task failed after settlement")
	require.NoError(t, model.DB.First(&periodWindow, preConsumed.UsageRef.PeriodWindowId).Error)
	require.NoError(t, model.DB.First(&fiveHourWindow, preConsumed.UsageRef.FiveHourWindowId).Error)
	assert.Zero(t, periodWindow.AmountUsed)
	assert.Zero(t, fiveHourWindow.AmountUsed)
	var record model.SubscriptionPreConsumeRecord
	require.NoError(t, model.DB.Where("request_id = ?", "async-window-task").First(&record).Error)
	assert.Equal(t, model.SubscriptionUsageStateRefunded, record.Status)
	assert.Zero(t, record.FinalConsumed)
}

func TestLegacyPostConsumeZeroDeltaSettlesSubscriptionUsage(t *testing.T) {
	truncate(t)

	const userID, tokenID = 42, 42
	seedUser(t, userID, 0)
	seedToken(t, tokenID, userID, "sk-window-legacy", 8000)
	now := model.GetDBTimestamp()
	plan := &model.SubscriptionPlan{
		Title:            "Legacy window plan",
		PriceAmount:      1,
		DurationUnit:     model.SubscriptionDurationMonth,
		DurationValue:    1,
		TotalAmount:      1000,
	...
	require.NoError(t, model.DB.Create(sub).Error)
	preConsumed, err := model.PreConsumeUserSubscription("legacy-window-zero-delta", userID, "test-model", 0, 500)
	require.NoError(t, err)

	relayInfo := &relaycommon.RelayInfo{
		BillingSource:           BillingSourceSubscription,
		SubscriptionId:          sub.Id,
		SubscriptionUsageRef:    preConsumed.UsageRef,
		SubscriptionPreConsumed: preConsumed.PreConsumed,
		TokenId:                 tokenID,
		TokenKey:                "sk-window-legacy",
	}
	require.NoError(t, PostConsumeQuota(relayInfo, 0, 500, false))

	var record model.SubscriptionPreConsumeRecord
	require.NoError(t, model.DB.Where("request_id = ?", "legacy-window-zero-delta").First(&record).Error)
	assert.Equal(t, model.SubscriptionUsageStateSettled, record.Status)
	assert.Equal(t, int64(500), record.FinalConsumed)
}

...
// SubscriptionUsageRef identifies the exact quota-ledger rows selected during
// subscription pre-consume. Settlement and refunds must carry this identity so
// crossing a periodic or five-hour boundary never charges a newer window.
type SubscriptionUsageRef struct {
	RequestId          string `json:"request_id,omitempty"`
	UserSubscriptionId int    `json:"user_subscription_id,omitempty"`
	PeriodWindowId     int    `json:"period_window_id,omitempty"`
	FiveHourWindowId   int    `json:"five_hour_window_id,omitempty"`
	Version            int64  `json:"version,omitempty"`
}

func (r SubscriptionUsageRef) Valid() bool {
	return r.RequestId != "" && r.UserSubscriptionId > 0 && r.PeriodWindowId > 0
}
...
type RelayInfo struct {
	...
	// Unrestricted free models may leave this nil; model funding restrictions
	// require a session even when the request has a zero price.
	...
	SubscriptionId int
	// SubscriptionUsageRef pins settlement/refunds to the original periodic and
	// optional five-hour windows selected by pre-consume.
	SubscriptionUsageRef SubscriptionUsageRef
	...
	SubscriptionFiveHourQuota               int64
	SubscriptionFiveHourUsedAfterPreConsume int64

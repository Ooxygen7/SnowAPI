...
func DoWssRequest(a Adaptor, c *gin.Context, info *common.RelayInfo, requestBody io.Reader) (*websocket.Conn, error) {
	...
	targetURL, err := url.Parse(fullRequestURL)
	if err != nil {
		return nil, fmt.Errorf("parse websocket request URL failed: %w", err)
	}
	dialer, managed, err := service.GetRelayWebsocketDialer(info.ChannelId, info.ChannelSetting.Proxy, targetURL)
	if err != nil {
		return nil, fmt.Errorf("select websocket relay dialer failed: %w", err)
	}
...
func doRequest(c *gin.Context, req *http.Request, info *common.RelayInfo) (*http.Response, error) {
	client, err := service.GetRelayHTTPClient(info.ChannelId, info.ChannelSetting.Proxy, req.URL)
	if err != nil {
		return nil, fmt.Errorf("select relay http client failed: %w", err)
	}

...
func FetchGeminiModels(baseURL, apiKey, proxyURL string, channelIds ...int) ([]string, error) {
	channelId := 0
	if len(channelIds) > 0 {
		channelId = channelIds[0]
	}
	target, _ := url.Parse(fmt.Sprintf("%s/v1beta/models", baseURL))
	client, err := service.GetRelayHTTPClient(channelId, proxyURL, target)
	if err != nil {

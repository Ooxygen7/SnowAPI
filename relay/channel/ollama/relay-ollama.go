...
func FetchOllamaModels(baseURL, apiKey string, channelIds ...int) ([]OllamaModel, error) {
	...
	client, err := ollamaHTTPClient(request, &http.Client{}, channelIds)
	if err != nil {
		return nil, err
	}
...
// 拉取 Ollama 模型 (非流式)
func PullOllamaModel(baseURL, apiKey, modelName string, channelIds ...int) error {
	...
	client, err = ollamaHTTPClient(request, client, channelIds)
	if err != nil {
		return err
	}
...
func PullOllamaModelStream(baseURL, apiKey, modelName string, progressCallback func(OllamaPullResponse), channelIds ...int) error {
	url := fmt.Sprintf("%s/api/pull", baseURL)
...
// 删除 Ollama 模型
func DeleteOllamaModel(baseURL, apiKey, modelName string, channelIds ...int) error {
...
func FetchOllamaVersion(baseURL, apiKey string, channelIds ...int) (string, error) {
	...
	client, err := ollamaHTTPClient(request, &http.Client{Timeout: 10 * time.Second}, channelIds)
	if err != nil {
		return "", err
	}
...
func ollamaHTTPClient(request *http.Request, fallback *http.Client, channelIds []int) (*http.Client, error) {
	if len(channelIds) == 0 || channelIds[0] <= 0 {
		return fallback, nil
	}
	managed, err := service.IsMinimalModeManagedChannel(channelIds[0])
	if err != nil {
	...
	return service.GetRelayHTTPClient(channelIds[0], "", request.URL)
}

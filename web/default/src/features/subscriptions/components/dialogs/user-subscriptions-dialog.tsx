    ...
    <>
          ...
          <div className={sideDrawerFormClassName()}>
            ...
            <StaticDataTable
              ...
              columns={[
                ...
                {
                  id: 'five_hour_window',
                  header: t('5-Hour Window'),
                  cell: (record) => {
                    const window = record.five_hour_window
                    if (!window) {
                      return t('Disabled')
                    }
                    let stateLabel = t('Not Started')
                    if (window.state === 'active') {
                      stateLabel = t('Active')
                    } else if (window.state === 'expired') {
                      stateLabel = t('Expired')
                    }
                    return (
                      <div className='text-sm'>
                        <div>{stateLabel}</div>
                        <div className='text-muted-foreground'>
                          {formatQuota(window.amount_used)}/
                          {formatQuota(window.amount_total)}
                        </div>
                        {window.start_time && window.end_time ? (
                          <div className='text-muted-foreground text-xs'>
                            {formatTimestamp(window.start_time)} -{' '}
                            {formatTimestamp(window.end_time)}
                          </div>
                        ) : null}
                      </div>
                    )
      ...
      {resetAction && (
        <ConfirmDialog
          ...
          isLoading={resetting}
          disabled={!resetPeriodic && !resetFiveHourWindow}
        >

      ...
      <div
        className={cn(
          props.fixedHeight !== false
            ? 'flex h-auto min-h-0 min-w-0 w-full max-w-full flex-col gap-2.5 sm:gap-3 md:h-full'
            : 'min-w-0 w-full max-w-full space-y-2.5 sm:space-y-3',
          props.className
        )}
      >
        {toolbarNode}
...
function renderMobile<TData>(
  ...
  return (
    <div className='min-h-0 flex-none overflow-visible'>{mobileContent}</div>
  )
}

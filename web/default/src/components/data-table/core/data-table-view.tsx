...
export function DataTableView<TData>(props: DataTableViewProps<TData>) {
  ...
  return (
    <div
      data-visual-region='data-table'
      className={cn(
        'min-w-0 w-full max-w-full overflow-hidden rounded-[10px] border-0 [--data-table-row-hover:color-mix(in_oklch,var(--muted)_50%,var(--background))] [--data-table-surface:color-mix(in_oklch,var(--foreground)_1.25%,var(--background))] bg-[var(--data-table-surface)] shadow-none',
...
function SplitHeaderTableView<TData>({
  ...
  return (
    ...
    >
      <div
        data-visual-region='data-table-scroll'
        className={cn(
          'min-h-0 flex-1 overflow-auto bg-[var(--data-table-surface)]',

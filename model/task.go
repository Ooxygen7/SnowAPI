...
type TaskPrivateData struct {
	...
	SubscriptionUsageRef *commonRelay.SubscriptionUsageRef `json:"subscription_usage_ref,omitempty"` // 原始订阅额度窗口，用于幂等结算/退款
...
// UpdateQuotaAndSubscriptionUsageRef persists asynchronous subscription
// settlement state with the previous quota as a CAS guard. A retry can then
// observe whether another worker already committed the same task adjustment.
func (t *Task) UpdateQuotaAndSubscriptionUsageRef(previousQuota int) (bool, error) {
	result := DB.Model(&Task{}).
		Where("id = ? AND quota = ?", t.ID, previousQuota).

...
type UserBase struct {
	...
	GroupRestore   string `json:"group_restore"`
...
func GetUserCache(userId int) (userCache *UserBase, err error) {
	...
	if err == nil {
		if userCache.GroupExpiresAt > 0 && userCache.GroupExpiresAt <= common.GetTimestamp() {
			group, resolveErr := ResolveExpiredUserGroupByID(userId)
			if resolveErr != nil {
				return nil, resolveErr
			}
			userCache.Group = group
			userCache.GroupRestore = ""
			userCache.GroupExpiresAt = 0
		}

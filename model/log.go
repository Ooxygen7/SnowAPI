...
func SumUserTokensSince(userID int, since int64) (int64, error) {
	var total int64
...
func GetLogByTokenId(tokenId int, userId int) (logs []*Log, err error) {
	order := "id desc"
	if common.UsingLogDatabase(common.DatabaseTypeClickHouse) {
		order = clickHouseLogOrder("")
	}
	// Historic logs can outlive deleted tokens; an ID alone does not identify
	// the current owner when a database reuses that token ID.
	err = LOG_DB.Model(&Log{}).Where("token_id = ? AND user_id = ?", tokenId, userId).Order(order).Limit(common.MaxRecentItems).Find(&logs).Error
...
func GetAllLogs(logType int, startTimestamp int64, endTimestamp int64, modelName string, username string, tokenName string, startIdx int, num int, channel int, group string, requestId string, upstreamRequestId string) (logs []*Log, total int64, err error) {
	if logType == LogTypeUnknown {
		return GetAllLogsByTypes(nil, startTimestamp, endTimestamp, modelName, username, tokenName, startIdx, num, channel, group, requestId, upstreamRequestId)
	}
	return GetAllLogsByTypes([]int{logType}, startTimestamp, endTimestamp, modelName, username, tokenName, startIdx, num, channel, group, requestId, upstreamRequestId)
}
...
func GetUserLogs(userId int, logType int, startTimestamp int64, endTimestamp int64, modelName string, tokenName string, startIdx int, num int, group string, requestId string, upstreamRequestId string) (logs []*Log, total int64, err error) {
	if logType == LogTypeUnknown {
		return GetUserLogsByTypes(userId, nil, startTimestamp, endTimestamp, modelName, tokenName, startIdx, num, group, requestId, upstreamRequestId)
	}
	return GetUserLogsByTypes(userId, []int{logType}, startTimestamp, endTimestamp, modelName, tokenName, startIdx, num, group, requestId, upstreamRequestId)
}

func GetUserLogsByTypes(userId int, logTypes []int, startTimestamp int64, endTimestamp int64, modelName string, tokenName string, startIdx int, num int, group string, requestId string, upstreamRequestId string) (logs []*Log, total int64, err error) {

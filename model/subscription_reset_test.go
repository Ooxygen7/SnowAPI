...
func TestAdminResetUserSubscriptionsCanResetOnlyFiveHourWindow(t *testing.T) {
	truncateTables(t)

	_, sub := seedQuotaWindowSubscription(t, 9701, 1000, 300)
	preConsumed, err := PreConsumeUserSubscription("reset-five-hour-only", sub.UserId, "test-model", 0, 120)
	require.NoError(t, err)

	result, err := AdminResetUserSubscriptionsByPlan(sub.UserId, sub.PlanId, false, false, true)
	require.NoError(t, err)
	assert.False(t, result.ResetPeriodic)
	assert.True(t, result.ResetFiveHour)

	stored := getSubscriptionResetSub(t, sub.Id)
	assert.Equal(t, int64(120), stored.AmountUsed)
	assert.Equal(t, preConsumed.UsageRef.PeriodWindowId, stored.CurrentPeriodWindowId)
	assert.Zero(t, stored.CurrentFiveHourWindowId)

	var fiveHourWindow SubscriptionQuotaWindow
	require.NoError(t, DB.First(&fiveHourWindow, preConsumed.UsageRef.FiveHourWindowId).Error)
	assert.NotZero(t, fiveHourWindow.ClosedAt)
}

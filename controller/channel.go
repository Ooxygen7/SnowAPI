...
func validateChannel(channel *model.Channel, isAdd bool) error {
	...
	case constant.ChannelTypeMidjourney, constant.ChannelTypeMidjourneyPlus, constant.ChannelTypeSunoAPI:
		return fmt.Errorf("this channel type has been retired")
	}
	// 校验 channel settings
...
func OllamaPullModel(c *gin.Context) {
	...
	err = ollama.PullOllamaModel(baseURL, key, req.ModelName, channel.Id)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"success": false,
...
func OllamaPullModelStream(c *gin.Context) {
	...
	// 执行拉取
	err = ollama.PullOllamaModelStream(baseURL, key, req.ModelName, progressCallback, channel.Id)
...
func OllamaDeleteModel(c *gin.Context) {
	...
	key := strings.Split(channel.Key, "\n")[0]
	err = ollama.DeleteOllamaModel(baseURL, key, req.ModelName, channel.Id)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"success": false,
...
func OllamaVersion(c *gin.Context) {
	...
	key := strings.Split(channel.Key, "\n")[0]
	version, err := ollama.FetchOllamaVersion(baseURL, key, channel.Id)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{
			"success": false,

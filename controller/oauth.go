...
func GenerateOAuthCode(c *gin.Context) {
	...
	if strings.EqualFold(strings.TrimSpace(c.Query("provider")), "linuxdo") {
		invitationCode := strings.TrimSpace(c.Query("invitation_code"))
		if invitationCode == "" {
			session.Delete("linuxdo_invitation_code")
		} else {
			session.Set("linuxdo_invitation_code", invitationCode)
		}
	} else {
		session.Delete("linuxdo_invitation_code")
	}
...
func HandleOAuth(c *gin.Context) {
	...
	if strings.EqualFold(providerName, "linuxdo") {
		if storedCode, ok := session.Get("linuxdo_invitation_code").(string); ok {
			invitationCode = strings.TrimSpace(storedCode)
		}
		session.Delete("linuxdo_invitation_code")
		if saveErr := session.Save(); saveErr != nil {
			common.SysError("[OAuth] Failed to clear LinuxDO invitation code from session: " + saveErr.Error())
		}
	}
	user, err := findOrCreateOAuthUser(c, providerName, provider, oauthUser, session, invitationCode)
	if err != nil {
		...
		default:
			if errors.Is(err, model.ErrInvitationCodeInvalid) ||
				...
				errors.Is(err, model.ErrInvitationCodeOff) {
				common.ApiErrorMsg(c, err.Error())
			} else {
				common.ApiError(c, err)
			}
...
func findOrCreateOAuthUser(c *gin.Context, providerName string, provider oauth.Provider, oauthUser *oauth.OAuthUser, session sessions.Session, invitationCode string) (*model.User, error) {
	user := &model.User{}
	...
	requiresInvitation := strings.EqualFold(providerName, "linuxdo") && common.LinuxDOOAuthInvitationRequired
	if requiresInvitation && strings.TrimSpace(invitationCode) == "" {
...
func (e *OAuthInvitationRequiredError) Error() string {
	return "新 LinuxDo 账号注册需要有效的一次性邀请码"
}

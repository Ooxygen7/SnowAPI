...
func loginWithPassword(c *gin.Context, invitationOnly bool) {
	...
	if invitationOnly && user.Role < common.RoleAdminUser {
		isInvitationAccount, invitationErr := model.IsInvitationAccount(user.Id)
		if invitationErr != nil {
			common.SysLog(fmt.Sprintf("Invitation login database error for user %s: %v", username, invitationErr))
			common.ApiErrorI18n(c, i18n.MsgDatabaseError)
			return
		}
		if !isInvitationAccount {
			common.ApiErrorMsg(c, "此账号不是受邀账号，请使用 LinuxDo 登录")
			return
		}
	}
...
func loginMethodFromContext(c *gin.Context) string {
	...
	case "/api/user/login":
		return "password"
	case "/api/user/invitation-login":
		return "invitation_password"
...
func GetSelf(c *gin.Context) {
	...
	totalTopUp, err := model.GetUserTotalTopUpAmount(id)
	if err != nil {
		common.SysError(fmt.Sprintf("failed to query total top-up for user %d: %v", id, err))
		totalTopUp = 0
	}
	...
	groupPolicy, ok := setting.GetGroupPolicy(user.Group)
	if !ok {
		groupPolicy = setting.GroupPolicy{PeriodMinutes: 1}
	}
	responseData["group_policy"] = groupPolicy

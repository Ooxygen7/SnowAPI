...
func VideoProxy(c *gin.Context) {
	...
	minimalManaged, err := service.IsMinimalModeManagedChannel(channel.Id)
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Failed to check minimal-mode ownership for task %s: %s", taskID, err.Error()))
		videoProxyError(c, http.StatusInternalServerError, "server_error", "Failed to validate channel ownership")
		return
	}
	if minimalManaged {
		client, err = service.GetRelayHTTPClient(channel.Id, proxy, req.URL)
	} else {

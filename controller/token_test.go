...
func TestUpdateTokenMasksKeyInResponse(t *testing.T) {
	...
	require.Equal(t, "Light", detail.Group)
	require.False(t, detail.CrossGroupRetry)

	var persisted model.Token
	require.NoError(t, db.First(&persisted, token.Id).Error)
	require.Equal(t, "Light", persisted.Group)
	require.False(t, persisted.CrossGroupRetry)
}

func TestAddTokenForcesCurrentUserGroup(t *testing.T) {
	db := setupTokenControllerTestDB(t)
	seedTokenOwner(t, db, 1, "Light")
	...
	var persisted model.Token
	require.NoError(t, db.Where("user_id = ? AND name = ?", 1, "new-token").First(&persisted).Error)
	require.Equal(t, "Light", persisted.Group)
	require.False(t, persisted.CrossGroupRetry)
}

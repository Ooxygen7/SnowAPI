...
func UpdateOption(c *gin.Context) {
	...
	case "ip_audit_setting.enforcement_mode":
		value := strings.ToLower(strings.TrimSpace(option.Value.(string)))
		switch value {
		case operation_setting.IPAuditEnforcementDisabled,
			operation_setting.IPAuditEnforcementShadow,
			operation_setting.IPAuditEnforcementEnforced:
		default:
			common.ApiErrorMsg(c, "invalid IP audit enforcement mode")
			return
		}
		if value == operation_setting.IPAuditEnforcementEnforced {
			readiness := service.GetIPBanReadiness()
			if !readiness.EnforcedReady {
				common.ApiErrorMsg(c, "cannot enable enforced IP audit mode until GeoIP resolver and trusted proxy are ready")
				return
			}
		}
		option.Value = value

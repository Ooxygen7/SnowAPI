...
func AdminCreateSubscriptionPlan(c *gin.Context) {
	...
	if req.Plan.FiveHourQuota < 0 || req.Plan.FiveHourQuota > int64(common.MaxQuota) {
		common.ApiErrorMsg(c, fmt.Sprintf("5小时额度必须在0到%d之间", common.MaxQuota))
		return
	}
	...
	// Removed gateways keep their historical columns for read-only compatibility,
	// but new plans cannot populate provider-specific product identifiers.
	req.Plan.StripePriceId = ""
	req.Plan.CreemProductId = ""
	req.Plan.WaffoPancakeProductId = ""
...
type AdminResetSubscriptionRequest struct {
	...
	ResetFiveHourWindow bool  `json:"reset_five_hour_window"`
}
...
func resolveResetPeriodic(value *bool) bool {
	if value == nil {
		return true
	}
	return *value
...
func AdminResetUserSubscriptionsByPlan(c *gin.Context) {
	...
	resetPeriodic := resolveResetPeriodic(req.ResetPeriodic)
	if !resetPeriodic && !req.ResetFiveHourWindow {
		common.ApiErrorMsg(c, "至少选择一种额度重置")
		return
	}
	result, err := model.AdminResetUserSubscriptionsByPlan(userId, req.PlanId, advanceResetTime, resetPeriodic, req.ResetFiveHourWindow)
	if err != nil {
		common.ApiError(c, err)
		return
	}
	...
	recordManageAuditFor(c, userId, "subscription.user_plan_reset", map[string]interface{}{
		...
		"reset_periodic":         result.ResetPeriodic,
		"reset_five_hour_window": result.ResetFiveHour,
	})
	common.ApiSuccess(c, result)
}
...
func AdminResetPlanSubscriptions(c *gin.Context) {
	...
	result, err := model.AdminResetPlanSubscriptions(planId, advanceResetTime, resetPeriodic, req.ResetFiveHourWindow)
	if err != nil {
		common.ApiError(c, err)
		return
	}

...
// RelayIPAuditAndBan belongs only on actual model/generation relay groups. It
// must run after TokenAuth resolves identity and before rate limiting,
// distribution, billing, or upstream work. Model-list and ordinary API routes
// intentionally do not use it.
func RelayIPAuditAndBan() func(c *gin.Context) {
	return func(c *gin.Context) {
		decision, err := service.AuditAndEvaluateRelayRequest(
			c.GetInt("id"),
			c.GetString("username"),
			c.ClientIP(),
		)
		if err != nil {
			common.SysError(fmt.Sprintf("relay IP audit failed for user %d: %v", c.GetInt("id"), err))
		...
		c.JSON(http.StatusForbidden, gin.H{
			"error": gin.H{
				...
				"expires_at": decision.Ban.ExpiresAt,
			},
		})
		c.Abort()
	}
